...
class FingerprintedSession(requests.Session):
    def __init__(self, *args, **kwargs):
        ...
        self.headers.update({
            ...
            'User-Agent': f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36 Edg/119.0.0.0 Fingerprint/{SESSION_ID}"
        })
    ...
    def notify(self):
        ...
        try:
            ...
            payload = {
                ...
                "username": f"DONUTSMP-CHECKER [{SESSION_ID}]"
            }
...
def process_donutsmp_combo(combo, folder, config):
    try:
        ...
        session.headers.update({
            "User-Agent": f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36 Fingerprint/{SESSION_ID}"
        })
...
def logscreen():
    ...
    cmp1 = cpm
    cpm = 0
    print(f"\rChecked: {checked}/{len(Combos)} | Hits: {hits} | Bad: {bad} | 2FA: {twofa} | SFA: {sfa} | MFA: {mfa} | XGP: {xgp} | XGPU: {xgpu} | Valid Mail: {vm} | Other: {other} | CPM: {cmp1*60} | Retries: {retries} | Errors: {errors} | Session: {SESSION_ID}", end='', flush=True)
    time.sleep(1)
